...
class NewsScanner:
    ...
    def _simple_th_title(self, en_title: str) -> str:
        """แปลหัวข้อข่าวแบบง่าย (keyword based) – สามารถต่อ LLM ได้ภายหลัง"""
        replacements = {
            "Fed": "เฟด",
            "FOMC": "FOMC",
            "interest rate": "อัตราดอกเบี้ย",
            "rate cut": "ลดดอกเบี้ย",
            "rate hike": "ขึ้นดอกเบี้ย",
            "CPI": "ดัชนีราคาผู้บริโภค (CPI)",
            "inflation": "เงินเฟ้อ",
            "unemployment": "อัตราว่างงาน",
            "Non-Farm Payrolls": "ตัวเลขจ้างงานนอกภาคเกษตร",
            "NFP": "NFP",
            "Powell": "พาวเวลล์",
            "holds rates": "คงดอกเบี้ย",
            "raises rates": "ขึ้นดอกเบี้ย",
            "cuts rates": "ลดดอกเบี้ย",
            "gold": "ทองคำ",
            "oil": "น้ำมัน",
            "sanctions": "คว่ำบาตร",
            "war": "สงคราม",
        ...
        th = en_title
        for en, th_word in replacements.items():
            th = re.sub(re.escape(en), th_word, th, flags=re.IGNORECASE)
        return th
